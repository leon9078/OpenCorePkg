...
## Description
This DXE driver tries to register keyboard shortcut (F6) handler for all text input devices. The handler tries to find a writable FS preferring OpenCore root if available, finds primary GOP device, takes screenshot from it and saves the result as PNG files on that writable FS.
...
## Usage
Load the driver, insert FAT32-formatted USB drive and press F6 to take screenshots from primary graphic console available at the moment.
